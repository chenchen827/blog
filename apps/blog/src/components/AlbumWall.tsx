...
function makeRingArcs({ radius, segmentCount, segmentPercent }: FocusRingConfig) {
  ...
  return Array.from({ length: segmentCount }, (_, index) => {
    ...
    return [arcPath(radius, center, center + halfSegment), arcPath(radius, center, center - halfSegment)];
  }).flat();
}
function AlbumFocusRings({ active }: { active: boolean }) {
  ...
  return (
    <svg aria-hidden="true" viewBox="0 0 600 600" className="album-focus-rings pointer-events-none absolute inset-[-1%] z-30 h-[102%] w-[102%] overflow-visible">
        ...
        {FOCUS_RINGS.map((ring) => (
          <g key={ring.radius} className="album-focus-ring">
            {makeRingArcs(ring).map((path, index) => (
              <path key={`${ring.radius}-${index}`} className={`album-focus-line album-focus-segment ${ring.className}`} d={path} pathLength="100" />
            ))}
          </g>
        ))}
      </g>
...
const AlbumDisc = memo(function AlbumDisc({
  album,
  albumIndex,
  copyIndex,
  active,
  ...
  const handleDiscPointerEnter = () => {
    ...
    activateTimerRef.current = window.setTimeout(() => {
      ...
      onActivate(album.id, copyIndex, track);
    }, 180);
  };
    ...
    <div className="flex h-[var(--album-size)] shrink-0 -rotate-45 items-center justify-center">
      ...
      >
        <Link
          ...
          onFocus={() => onActivate(album.id, copyIndex, track)}
        ...
        >
          <div className="album-disc-well absolute inset-[14%] overflow-hidden rounded-full border bg-surface-soft">
            {album.coverUrl ? (
              <img src={album.coverUrl} alt="" loading="lazy" draggable={false} className="album-cover-image" />
            ) : (
          ...
          <span className="pointer-events-none absolute inset-[14%] rounded-full bg-gradient-to-t from-black/18 via-transparent to-white/20" />
...
export default function AlbumWall({ albums }: AlbumWallProps) {
  ...
  const activeIndex = Math.max(
    0,
    albums.findIndex((album) => album.id === activeSelection.albumId),
  );
  ...
  useEffect(() => {
    animateRef.current = () => {
      ...
      currentOffsetRef.current += velocity;
      velocityRef.current = velocity * 0.94;
      paint();
  ...
  useEffect(() => {
    ...
    const handleWheel = (event: WheelEvent) => {
      ...
      const impulse = event.deltaY * unit * 0.22;
      velocityRef.current = Math.max(-80, Math.min(80, velocityRef.current + impulse));
      startAnimation();
    };
  ...
  const handlePointerMove = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    const verticalDistance = drag.y - event.clientY;
    const horizontalDistance = (event.clientX - drag.x) * 0.35;
    ...
    if (Math.abs(distance) > 1.5) {
      ...
      currentOffsetRef.current += distance;
      velocityRef.current = Math.max(-60, Math.min(60, distance * 0.72));
  ...
  const finishPointerDrag = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    if (performance.now() - lastMoveAtRef.current < 90 && Math.abs(velocityRef.current) > 0.04) {
      startAnimation();
    } else {
      velocityRef.current = 0;
    }
  ...
  const activateDisc = useCallback((albumId: number, copyIndex: number, track: TrackSide) => {
    setActiveSelection((current) => (current.albumId === albumId && current.copyIndex === copyIndex && current.track === track ? current : { albumId, copyIndex, track }));
  }, []);
  ...
  return (
    ...
    >
      ...
      <div className="album-info-panel pointer-events-none absolute left-4 top-5 z-30 md:left-7 md:top-7">
        ...
        <h1
          key={activeAlbum.id}
          className="album-info-copy album-copy-enter mt-3 max-w-full text-2xl font-black uppercase leading-[0.92] tracking-[-0.035em] text-text-primary md:text-5xl"
        >
          {activeAlbum.name}
        </h1>
