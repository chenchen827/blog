...
function PhotoArchiveCard({ photo, index, albumName, className }: { photo: PhotoWallPhoto; index: number; albumName: string; className?: string }) {
...
export default function PhotoWall({ photos, albumName = "相片档案" }: { photos: PhotoWallPhoto[]; albumName?: string }) {
  if (photos.length === 0) {
    return (
    ...
    <Image.PreviewGroup>
      <div className="grid auto-rows-[150px] grid-cols-2 gap-3 sm:auto-rows-[180px] lg:auto-rows-[220px] lg:grid-cols-4">
        {photos.map((photo, index) => (
          <PhotoArchiveCard key={photo.id} photo={photo} index={index} albumName={albumName} className={getPhotoLayout(index)} />
        ))}
      </div>
    </Image.PreviewGroup>
  );
