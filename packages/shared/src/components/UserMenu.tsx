    ...
    >
      ...
      >
        <span className="relative flex h-8 w-8 items-center justify-center overflow-hidden rounded-none border border-accent/40 bg-surface-soft text-sm font-black text-accent">
          {user?.avatar ? (
            <img src={user.avatar} alt={user?.name ?? '用户头像'} className="h-full w-full object-cover" />
          ) : (
            initial
          )}
        </span>
      ...
      <div
        className={cn('absolute right-0 top-full z-50 pt-2 transition-all duration-200', open ? 'pointer-events-auto' : 'pointer-events-none')}
        aria-hidden={!open}
      >
        <div
          className={cn(
        ...
        >
          <div className="flex items-center gap-3 border-b border-hairline p-4">
            ...
            <div className="min-w-0">
              ...
              {user && (
                <span className="mt-1 inline-block bg-accent/10 px-2 py-0.5 text-[10px] font-black uppercase tracking-wider text-accent">
                  {roleText(user.role)}
                </span>
              )}
            </div>
