  ...
  return (
    <div className="relative flex h-full w-full flex-col justify-between overflow-hidden p-5">
      ...
      <div className="relative flex flex-1 items-center justify-center">
        <span aria-hidden="true" className="select-none text-[6rem] font-black leading-none text-transparent [-webkit-text-stroke:1.5px_#2c2c2c]">
          {no}
        </span>
      </div>
      ...
      >
        ...
        >
          ...
          {pages.map((page, index) => {
            ...
            const fanOffset = expanded && (single || !isCover) ? (isCurrent || single ? ` translateZ(${-TOP_DEPTH}px)` : ` translateZ(${BACK_DEPTH}px)`) : "";
            const duration = isCover ? COVER_DURATION : FLIP_DURATION;
            const delay = expanded ? (isCover ? 0 : index * STAGGER) : isCover ? 0 : (count - 2 - index) * STAGGER;
            ...
            return (
              ...
              >
                {/* 正面（封面） */}
                <div
                  className={cn("absolute inset-0 overflow-hidden border border-hairline bg-surface text-text-primary", pageClassName, page.frontClassName)}
